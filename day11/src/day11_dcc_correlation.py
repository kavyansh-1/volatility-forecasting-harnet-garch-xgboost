# Day 11 – Dynamic Conditional Correlation (DCC) implementation
...
def compute_univariate_ewma_vol(returns: pd.DataFrame, lam: float = EWMA_LAMBDA) -> pd.DataFrame:
    ...
    returns: pd.DataFrame
        Raw asset returns.
    lam: float, optional
        Decay factor (default EWMA_LAMBDA).
    """
    r = returns.values
    n, k = r.shape
    ...
    return pd.DataFrame(vols, index=returns.index, columns=returns.columns)


def dcc_recursion(e: np.ndarray, a: float, b: float, Qbar: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Recursively compute conditional covariance (Q) and correlation (R) matrices.
    """
    T, k = e.shape
    Q_t = Qbar.copy()
...
def grid_search_doc_params(e: np.ndarray, Qbar: np.ndarray, a_grid: list = None, b_grid: list = None) -> Tuple[Tuple[float, float], float]:
    if a_grid is None:
...
def run_dcc_analysis() -> dict:
    ...
    print(f"Standardised DCC inputs shape : {e_fit.shape}")
    print("Grid Searching DCC Parameters (a, b)...")
    (a, b), best_ll = grid_search_doc_params(e_fit, Qbar)
    print(f"Best DCC params : a={a:.4f}, b={b:.4f} (persistence a+b = {a+b:.3f}) loglik = {best_ll:.2f}\n")

    Q_list, R_list = dcc_recursion(e_fit, a, b, Qbar)
    ...
    pd.DataFrame([
        {"a": a, "b": b, "persistence": a + b, "loglik": best_ll}
    ]).to_csv(os.path.join(OUT_DIR, "day11_dcc_params.csv"), index=False)
    pd.DataFrame(R_next, index=TICKERS, columns=TICKERS).to_csv(
        os.path.join(OUT_DIR, "day11_dcc_next_day_forecast.csv")
    )

    print(f"\n [OK] day11_dcc_correlations.csv ({len(dcc_corr_df)} rows)")
    print("[OK] day11_dcc_params.csv")
    print("[OK] day11_dcc_next_day_forecast.csv")
